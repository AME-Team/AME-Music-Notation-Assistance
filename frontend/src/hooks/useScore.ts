 ...
 *   初期値は`true`のまま維持する(#32-M3レビュー指摘、2巡目で`false`初期化を
 *   提案されたが検討の結果不採用: `undo_state.json`はディスク上に永続化され
 *   セッションをまたぐため、前回セッションで作った編集履歴が残っている
 *   プロジェクトを再度開いた場合、`false`初期化だと今回のセッションで
 *   新規編集を1回もしていない限りボタンが恒久的に無効化され、既存の
 *   Undo履歴に一切アクセスできなくなる。`true`初期化なら、履歴が実際に
 *   無い場合の初回クリックが1回無駄になるだけで、既存履歴へは常に
 *   アクセスできる。後者の方がトレードオフとして優れると判断した)。
 */
