...
import type { BeatSummary } from "./beatSummary";
...
/** 現在の拍子の割り当てを表す文言(例: `4/4(小節10〜10 は 2/4)`)。 */
export function currentMeterLabel(summary: BeatSummary): string {
  if (!summary.timeSignature) return "—";
  if (summary.timeSignatureChanges.length === 0) return summary.timeSignature;

  const deviations = summary.timeSignatureChanges
    .map((change) => `${change.bar}〜${change.endBar} は ${change.numerator}/${change.denominator}`)
    .join("、");
  return `${summary.timeSignature}(${deviations})`;
}
